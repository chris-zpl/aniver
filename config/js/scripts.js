...
function adicionarIdEAnimar() {
  ...
  // Dá um setTimeout após 6 segundos depois de virar um card
  setTimeout(()=>{
    anim();
  },6000);
}
...
// Muda o conteúdo dentro dos cards 
function mudarPresente(elemento) {
  // Verifica se a carta já foi clicada antes
  if (!elemento.classList.contains('clicada')) {
    // Marca a carta como clicada
    elemento.classList.add('clicada');
  // Encontra o elemento de texto dentro da carta clicada
  const textoElemento = elemento.querySelector('#presente');
  ...
  // Define o novo texto para o elemento de texto da carta clicada
  textoElemento.textContent = novaMensagem;
}}
